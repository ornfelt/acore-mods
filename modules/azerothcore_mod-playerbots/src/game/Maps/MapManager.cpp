...
ContinentArea MapManager::GetContinentInstanceId(uint32 mapId, float x, float y, bool* transitionArea)
{
    ...
    case 530:
    {
        ...
        if (x > 5800)
            return MAP530_BLOODELF;
        if (y < -8600)
            return MAP530_DRAENEI;
        if (IsNorthTo(x, y, BladeEdgeNetherSouthLimit, sizeof(BladeEdgeNetherSouthLimit) / (2 * sizeof(float))))
            return MAP530_NORTH;
        if (IsNorthTo(x, y, HellfireZangarSouthLimit, sizeof(HellfireZangarSouthLimit) / (2 * sizeof(float))))
            return MAP530_MIDDLE;
        if (IsNorthTo(x, y, ShattrathAreaSouthLimit, sizeof(ShattrathAreaSouthLimit) / (2 * sizeof(float))))
            return MAP530_SHATTRATH;
        return MAP530_SOUTH;
    }
    case 571:
    {
        static float const DalaranAreaSouthLimit[] = {
            5606.0f,   1458.0f,
            5100.0f,   775.0f,
            5756.0f,   -659.0f
        };
        static float const IcecrownStormpeaksSouthLimit[] = {
            7300.0f,   5331.0f,
            6889.0f,   4697.0f,
            6797.0f,   4193.0f,
            6250.0f,   3358.0f,
        ...
        if (IsNorthTo(x, y, IcecrownStormpeaksSouthLimit, sizeof(IcecrownStormpeaksSouthLimit) / (2 * sizeof(float))))
            return MAP571_NORTH;
        if (IsNorthTo(x, y, DalaranAreaSouthLimit, sizeof(DalaranAreaSouthLimit) / (2 * sizeof(float))))
            return MAP571_DALARAN;
        return MAP571_SOUTH;
    }
    }
    return MAP_NO_AREA;
